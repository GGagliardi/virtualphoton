...
inline PrecFloat fact(long unsigned int n) {
  PrecFloat out;
...
PrecFloat ExpEiComplexSum(PrecFloat MOD, PrecFloat PH, PrecFloat s,  bool MODE) {
  ...
  if(!Arg_zero) {
    ...
    while( ! converged) {
      ...
      vector<PrecFloat> Kah_sum;
      Kah_sum.push_back(START_SUM);

      PrecFloat MOD_TERM_k= MOD;
      PrecFloat MOD_TERM_k1= MOD_TERM_k*((MODE==0)?sin(s- PH):cos(s + PH));
      Kah_sum.push_back(MOD_TERM_k1);
      for(unsigned long int c=2; c<=k;c++) {

	MOD_TERM_k *=  MOD*(c-1)/(PrecFloat(c)*PrecFloat(c));
	PrecFloat MOD_TERM_tot = MOD_TERM_k*((MODE==0)?sin(s- c*PH):cos(s + c*PH));
	Kah_sum.push_back( MOD_TERM_tot);
      }

      SUM= Kahan_sum(Kah_sum);
